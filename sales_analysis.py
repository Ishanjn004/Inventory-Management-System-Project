...
for product, data in sales_data.items():
    recent_sales = [sales for date, sales, _ in data if date.strftime("%Y-%m-%d") in last_14_days]

    # **Exclude products that have no sales in the last 14 days**
    ...
    for i in range(7):  # Predict next 7 days
        predicted_sales = max(1, avg_sales*0.5)  #* Ensure at least 1 sale per day
...
class sales_analysis_Class:
    ...
    def plot_graph(self, parent_frame):
        """Plot the past 14 days of sales for all products and enable click event."""
        fig, ax = plt.subplots(figsize=(7, 4))

        days = [(today - timedelta(days=i)).strftime("%d-%b") for i in range(13, -1, -1)]                #* plotttttt

        for product, sales in past_sales_over_time.items():
            ax.plot(days, sales[-14:], marker="o", linestyle="-", label=product)
    ...
    def open_detailed_graph(self):
        """Opens a new window with a detailed sales graph for the last 30 days."""
